...
function runBuiltCli(args: string[]): { status: number | null; firstLine: string } {
  const home = mkdtempSync(join(tmpdir(), "cswarm-command-table-"));
  const result = (() => {
    try {
  ...
  return {
    ...
    firstLine: `${result.stdout}${result.stderr}`.split("\n", 1)[0]!,
  };
}
...
test("listen handler shapes and help share accepted flags including state-dir", { timeout: 10_000 }, async () => {
  ...
  const statusOrStop = ast.statements.find(node => ts.isFunctionDeclaration(node) && node.name?.text === "runListenStatusOrStop");
  assert.ok(statusOrStop && ts.isFunctionDeclaration(statusOrStop));
  const statusOrStopCalls: string[] = [];
  const visitStatusOrStop = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && node.expression.getText(ast) === "args.assertShape") {
      statusOrStopCalls.push(node.arguments[0]!.getText(ast));
    }
    ts.forEachChild(node, visitStatusOrStop);
  };
  visitStatusOrStop(statusOrStop);
  assert.deepEqual(statusOrStopCalls, [
    "LISTEN_STOP_ACCEPTED_FLAGS",
    'command === "status" ? LISTEN_STATUS_ACCEPTED_FLAGS : LISTEN_STOP_ACCEPTED_FLAGS',
  ]);
  assert.match(
    statusOrStop.getText(ast),
    /if \(command === "status" && args\.has\("wait"\)\)[\s\S]*?throw new Error\("--wait is only valid for listen stop"\)/,
  );
  for (const [action, flags] of [
    ...
    ["stop", LISTEN_STOP_ACCEPTED_FLAGS],
  ...
  const statusHelp = commandHelpLines("listen", "status");
  assert.doesNotMatch(statusHelp.split("\n", 1)[0], /--state-dir/);
  assert.match(statusHelp, /Additional options: .*--state-dir <value>/);
  assert.doesNotMatch(commandHelpLines("listen", "start"), /--session-context/);
});

test("mcp connect help covers normal and interrupted-connect forms", () => {
  const help = commandHelpLines("mcp", "connect");
  assert.match(help, /cswarm mcp connect --url <url>/);
  assert.match(help, /cswarm mcp connect --clear-pending --profile <absolute-path>/);
  assert.match(
    help,
    /Redeem a connect code or clear an interrupted connect on the agent host\./,
  );
});

test("listen status --wait keeps its dedicated refusal through the real parser", () => {
  const result = runBuiltCli([
    "listen", "status", "--wait", "5",
    "--workspace-id", "11111111-1111-4111-8111-111111111111",
  ]);
  assert.equal(result.status, 1);
  assert.equal(result.firstLine, "cswarm: --wait is only valid for listen stop");
});

test("listen status validates flags after --wait before its dedicated refusal", () => {
  const result = runBuiltCli([
    "listen", "status", "--wait", "5", "--bogus", "x",
  ]);
  assert.equal(result.status, 1);
  assert.equal(result.firstLine, "cswarm: unknown option: --bogus");
});
