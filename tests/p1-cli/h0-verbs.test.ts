...
test("the SERVED DOCUMENT equals its golden, line for line", () => {
  ...
  const GOLDEN = [
    ...
    "    attemptId (required) — client-generated; retry with the same value recovers this seat and replaces an unused token",
    ...
    "    body (required)",
    "    to (may be omitted)",
    "    requestId (may be omitted) — reuse the same value when retrying this post",
