...
test("the SERVED DOCUMENT equals its golden, line for line", () => {
  ...
  const GOLDEN = [
    ...
    "    attemptId (required) — client-generated; retry with the same value while its token is unused to recover this seat and replace that token; a used token returns 409, so ask the inviter for a new join credential",
    ...
    "    listener_instance_id (required, may be null) — null only when outcome is `observed`; otherwise the listener_instance_id poll returned",
    "    outcome (required)",
