 ...
 * (supabase/functions/command/index.ts:3163-3174): the device row must exist, be owned by the
 ...
 * (supabase/functions/command/index.ts:9808-9812), so the two facts come from one place. */
...
function assertAccepted(step: ConnectStep, outcome: CommandOutcome): Record<string, unknown> {
  ...
  if (status === 400) {
     ...
     * (supabase/functions/command/index.ts:2407-2415) — so the NEWER-page case is the one a
 ...
 * is the scope check at supabase/functions/command/index.ts:8234-8251, which is what
 * `post_signal` needs.
 *
 ...
 * (supabase/functions/command/index.ts:9808-9812) — an ISO string for timeboxed, null for
 * standing — and the idempotent replay path carries it through (index.ts:2634-2637).
 *
