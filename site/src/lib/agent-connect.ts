 ...
 * (supabase/functions/command/index.ts:3239-3248): the device row must exist, be owned by the
 ...
 * (supabase/functions/command/index.ts:10830-10835), so the two facts come from one place. */
...
function assertAccepted(step: ConnectStep, outcome: CommandOutcome): Record<string, unknown> {
  ...
  if (status === 400) {
     ...
     * (supabase/functions/command/index.ts:2467-2483) — so the NEWER-page case is the one a
 ...
 * is the scope check at supabase/functions/command/index.ts:9244-9250, which is what
 * `post_signal` needs.
 *
 ...
 * (supabase/functions/command/index.ts:10830-10835) — an ISO string for timeboxed, null for
 * standing — and the idempotent replay path carries it through (index.ts:2705-2708).
 *
