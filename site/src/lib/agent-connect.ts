 ...
 * (supabase/functions/command/index.ts:10787-10792), so the two facts come from one place. */
...
function assertAccepted(step: ConnectStep, outcome: CommandOutcome): Record<string, unknown> {
  ...
  if (status === 400) {
     ...
     * (supabase/functions/command/index.ts:2458-2474) — so the NEWER-page case is the one a
