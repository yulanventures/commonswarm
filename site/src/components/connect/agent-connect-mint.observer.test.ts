 ...
 *      (supabase/functions/command/index.ts:2447-2449, 3407-3410), which is the month every
 *      web-added agent used to die after.
 *   2. `renewal_horizon_ms` must be ABSENT from the JSON. The validator accepts standing only
 *      when the key is `undefined` (index.ts:2454); a present `null` fails validation and
 *      returns 400 (index.ts:2508), even though the wire TYPE declares `number | null`.
...
test("the mint still sends the device the standing grant will be bound to", async () => {
  await mint();
  // index.ts:4610 binds a standing grant to this id, so an app mint can never read UNBOUND.
