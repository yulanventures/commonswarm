 ...
 *      (supabase/functions/command/index.ts:2468-2470, 3427-3429), which is the month every
 *      web-added agent used to die after.
 *   2. `renewal_horizon_ms` must be ABSENT from the JSON. The validator accepts standing only
 *      when the key is `undefined` (index.ts:2469); a present `null` fails validation and
 *      returns 400 (index.ts:2523), even though the wire TYPE declares `number | null`.
...
test("the mint never puts renewal_horizon_ms on the wire, not even as null", async () => {
  ...
  assert.equal(
    ...
    false,
    "a present null is refused with 400 by index.ts:2404",
  );
...
test("the mint still sends the device the standing grant will be bound to", async () => {
  await mint();
  // index.ts:4625 binds a standing grant to this id, so an app mint can never read UNBOUND.
