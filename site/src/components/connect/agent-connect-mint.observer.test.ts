 ...
 *      (supabase/functions/command/index.ts:2394-2396, 3351-3354), which is the month every
 *      web-added agent used to die after.
 *   2. `renewal_horizon_ms` must be ABSENT from the JSON. The validator accepts standing only
 *      when the key is `undefined` (index.ts:2401); a present `null` fails validation and
 *      returns 400 (index.ts:2455), even though the wire TYPE declares `number | null`.
...
test("the mint still sends the device the standing grant will be bound to", async () => {
  await mint();
  // index.ts:4551 binds a standing grant to this id, so an app mint can never read UNBOUND.
