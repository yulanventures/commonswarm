 ...
 *      (supabase/functions/command/index.ts:2380-2382, 3337-3340), which is the month every
 *      web-added agent used to die after.
 *   2. `renewal_horizon_ms` must be ABSENT from the JSON. The validator accepts standing only
 *      when the key is `undefined` (index.ts:2387); a present `null` fails validation and
 *      returns 400 (index.ts:2441), even though the wire TYPE declares `number | null`.
...
test("the mint never puts renewal_horizon_ms on the wire, not even as null", async () => {
  ...
  assert.equal(
    ...
    false,
    "a present null is refused with 400 by index.ts:2387",
  );
