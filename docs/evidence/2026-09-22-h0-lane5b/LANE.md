...
`payload_too_large`. Routing-lookup database errors use the same SQLSTATE
40001/40P01 retryable 503 classifier as poll and ack. A serialization failure
or deadlock inside the command transaction still takes the command edge's 500
path, which H0 passes through unchanged.
...
an attempt recovers a seat only while its token is unused. The Fold 1 used-token
remedy was incorrect and is corrected in Fold 2.
...
The `attemptId` note now says a used or revoked seat returns 409 and instructs
the caller to follow the response message. The command edge's two 409 messages
are exported from `registration-conflicts.ts` and used at its refusal sites.
The document test checks both edge response statuses and that the note defers
to their messages; the served OpenAPI golden also pins the rendered text.
The server tests compare the actual used-token and revoked-seat messages with
those constants.

The H0 worker now receives the command environment minus
`SUPABASE_SERVICE_ROLE_KEY`. Only file commands call `fileStorage()`, and H0's
closed verb map emits only `register_agent_seat` and `post_signal`. The command
module can be imported with that key absent: it reads the key only when a file
command reaches `fileStorage()`. The box environment test checks the exact
command-minus-exclusion set. The command's import-time required configuration
names live in `command/required-config.ts`; both command initialization and
H0's pre-import check use its resolver.

`COMMAND_ID_RE` in the command edge now comes from `H0_REQUEST_ID_RE` in
`src/h0/verbs.ts`, so that shared pattern applies to every command client,
including CLI and web, not just H0. The parity test compares only rows in
`swarm.signals`; it does not compare `swarm.signal_recipients` or delivery rows.
For forwarded H0 traffic, command-edge logs come from the H0 worker; durable
command-failure rows are unchanged. H0 now has a `deno.lock`, generated by
`deno check --lock deno.lock index.ts` from the H0 function directory, following
the command function's lockfile pattern.

### Fold 2 verification

The H0 document, box environment, and forwarding-contract focused tests passed
49/49. A separate Deno import of `command/index.ts` passed with
`SUPABASE_SERVICE_ROLE_KEY` unset, cached dependencies only, and network
permission denied (exit 0). It initialized the module without serving or
connecting. The new H0 lockfile equals command's lockfile byte for byte.
Moving the command imports changed source-line citations; the citation-drift
test and the acceptable-use citation test now pass after those pointers were
updated.

| Gate | Exit | Measured result |
|---|---:|---|
| `npm run build` | 0 | TypeScript CLI build completed. |
| `env -u FORCE_COLOR npm test` | 1 | 895/897 pass; two sandbox `spawn EPERM` resume-process failures. |
| `env -u FORCE_COLOR npm run test:p1-cli` | 1 | 821/839 pass; 18 sandbox lock/process and credential-store fixture failures, matching Fold 1 categories. |
| `npm run check:tests` | 0 | Test and source typecheck completed. |
| `npm run check:edge` | 0 | Five edge entry points checked. |
| `env -u FORCE_COLOR npm run test:p1-server` | 0 | 242/242 pass, run alone. |
| `bash scripts/build-release.sh` | 0 | Built and executed the 0.1.72 single-file CLI. |
| `git diff --check origin/main...HEAD` | 0 | Checked after the Fold 2 commit. |

This fold did not establish the box worker's actual environment, loaded-lockfile
behavior in the box runtime, live database connection headroom, memory or cold
start, the three-host done-test, or parity of `swarm.signal_recipients` and
delivery rows. No production release or deployment was performed.
