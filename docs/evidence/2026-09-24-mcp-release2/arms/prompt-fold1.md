Fold 1 on item H release 2 (option A) in the CommonSwarm repo, branch lane/mcp-connect at da5fa657. Spec:
docs/design/2026-09-24-MCP-RELEASE-2-BRIEF.md. Strategist ruling A conditions: connect refuses to run without a TTY; the
code is single-use; the printed lines name the profile path only. Both review arms FAILED; their reviews are pasted at the
end, and the lead's full gates found two more failures (below). Implement EVERY ruling, each with a test that fails when
the fix is reverted; record each ruling, test and mutation result in docs/evidence/2026-09-24-mcp-release2/LANE.md under
"Fold 1". Read AGENTS.md first (D-053, generated enumerations, claim controls, honesty is not sufficient).

HARD RULES: contact no production host (api.commonswarm.com, commonswarm.com, edge-staging.commonswarm.com,
178.105.29.28, 100.115.66.74). A cswarm command run without a loopback --url contacts commonswarm.com: never run one,
in tests always use loopback fakes. No supabase, vercel, ssh; no swarm/cswarm against a real workspace; do not touch
~/.cswarm or ~/.config/cswarm (temporary HOME in tests). NO server, edge or migration change. Every test has a timeout;
no process of yours survives. SANDBOX: network and the repository .git are writable; commit yourself with subject
"fix(mcp): ..." / "test(mcp): ..." and trailers exactly (one per line): Agent-Name: Yulan Bot / Agent-Model: <your model
id> / Agent-Family: openai / Agent-Tool: codex <version> / Agent-Model-Source: runtime-ambiguous.

RULINGS
K1 (Grok 1, PRODUCTION). The CLI's option parser must never echo an option's VALUE in an error: `--code=<secret>` today
   prints "invalid option: --code=<secret>" (src/cli.ts ~687). Print only the option name (the part before "=").
   Test: `mcp connect ... --code=swm_join_<43 chars>` and the same on two other commands: stderr and stdout never contain
   the value. Regenerate the dispatch baseline if rows change and list them.
K2 (Grok 2, Opus 1, PRODUCTION). No stranded seat without a revoke instruction:
   - Before the prompt: create/verify the profile directory as private (0700, owned, not a symlink) and writable, and
     refuse a --profile whose file name is `credential.json` or whose credential path would equal the profile path.
   - Everything after the register request has been SENT is one class: unless the response is a typed server refusal
     that proves the code was not used (enumerate those codes from the h0 register handler in supabase/functions/h0/
     and command/index.ts — generate the set, do not type it; remove any code the server never sends from tests), the
     error is `register_outcome_unknown` / a single sentence: "The seat may have been created. Ask the operator to
     revoke it with cswarm principal revoke and issue a new code." This covers network errors, 5xx, non-JSON or
     truncated 200 bodies, invalid bodies, and ANY failure in saving the files (including a lost race for the path).
   - Tests for each class with a loopback fake that commits; mutation: a bare save failure without the sentence fails.
K3 (lead gates). The timeout-table tests fail: the inventory is missing src/cloud/mcp-connect.ts setTimeout/timeout.
   Register the new timeout in the timeout table the way the other network timeouts are (read
   tests/p1-cli/timeout-table.test.ts and the table source), with its value from a named constant.
K4 (lead gates, Opus). Site: two site tests fail (src/components/app/access-lifecycle.observer.test.ts "Add an agent
   asks only for a name and warns about the key when it exists", and the workspace-entry test "the live dashboard offers
   peer agent and collaborator paths from an empty workspace") because the lane changed the button text and the warning.
   The warning's new word "fallback" is false on that screen (it offers only the prompt; the connect code is CLI-only):
   say "This prompt or setup file carries a secret through the model. Keep it private..." and name the MCP connect path
   as the way to keep the secret out of the model. Restore the npm fallback sentence the lane removed from the prompt
   (Opus). Make the shown Codex config valid TOML (Opus). Update the two tests to the new true text (read them; do not
   weaken what they check). Run `npm --prefix site run build` then `npm --prefix site test`.
K5 (Opus, claim). The TTY rule stops a plain pipe, /dev/null and closed stdin; a same-user process can allocate a
   pseudo-terminal (`script`) and pass it. Correct every claim that says an agent "cannot" pipe the code (code
   comments, test comments, LANE.md, error sentences) to exactly that. Keep the refusal.
K6 (Opus). At the prompt: Ctrl-D / EOF or an empty line -> exit 1 with `code_missing` and "No code was entered. Run
   mcp connect again." (never exit 0). Trim surrounding whitespace from the pasted code before validation. Echo is
   restored on SIGINT and SIGTERM by the lane itself (handler restores echo, then exits 130/143), and the handler is
   removed on every exit path. If stty is missing or fails, refuse before printing the prompt. Tests with an injectable
   terminal layer (no real TTY needed): EOF, empty, whitespace-padded code, echo restored after an error thrown after
   echo-off, and the signal handler restoring echo.
K7 (Opus). Wrong advice: map the server's typed register refusals to true next steps (upgrade_required -> "Update
   cswarm and run mcp connect again; the code was not used"; the workspace principal limit -> "The workspace has no free
   agent seat. Ask the operator"; an unknown code at the wrong --url (404 / not the h0 function) -> "Check --url; the code
   was not used" ONLY when the response proves the request never reached register). Messages are chosen by status and
   typed code, never by message text (D-053).
K8 (Opus). The register request must not follow redirects (`redirect: "error"`); a redirect is `register_redirected`,
   code not sent further. Test with a 308 fake.
K9 (Opus). A new machine: `mcp code` prints, after the code and its expiry, the exact connect line to run on the agent
   host: `cswarm mcp connect --url <url> --anon-key <public anon key>` (no secret in it; the anon key is public).
   `mcp connect` without --anon-key and with no saved target for that URL refuses with a sentence that names --anon-key
   only (not SWARM_CLOUD_ANON_KEY, which connect ignores). Tests.
K10 (Opus, Grok 4). LANE.md: remove the "prints the principal ID" claim, update the bundle SHA after the final build,
   report the npm sentence restore, and correct every other claim the reviews found stale.
K11 (Opus). Dispatch fixture ids: rows named `mcp.serve` that run the refusal path and the row named refusal that runs
   serve must be named for what they run. Regenerate twice (byte-identical) and list the changed rows.
...
REPORT: commit SHAs; each ruling -> change -> test -> mutation; gates with counts; not established.
