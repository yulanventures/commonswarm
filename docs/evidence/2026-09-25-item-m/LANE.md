...
The original sections below record historical lane states. Fold 12 at the end is the current client rule; earlier credential deletion, mode, fresh-register escape, revocation-advice, and completed-profile rows are superseded where marked.
...
| V3 — narrowed by Fold 13 TT1 | A completed profile and credential beside pending are validated and cleaned without another POST only when the profile carries that pending attempt ID. | `completed profile clears old pending without POST` uses a record over two hours old; disabling completed-profile recovery: exit 1, 0/1. Fold 13 tests a foreign profile and this attempt's profile. |
...
| Ruling | Change | Focused test | Measured single-change revert |
|---|---|---|---|
| SS1 — production; narrowed by Fold 13 TT1 | `completedProfileAt` returns the validated profile read with its own `host_session_id`. The pending-record branch uses that result. Fold 13 allows adoption only when `connect_attempt_id` matches pending. | `Fold 12 pending resume keeps a host-bound working profile from this attempt` checks zero resume POSTs, pending removal, and byte-identical profile. | Restoring the old branch that discards `completedProfileAt` and calls `readAgentProfile(path)` without the host ID: exit 1, targeted test 0/1 at Fold 12. |
...
| Ruling | Change | Focused test | Measured single-change revert |
|---|---|---|---|
| TT1 — supersedes the broad adoption claim in V3 and SS1 | Connect writes `connect_attempt_id` to `profile.json`, equal to pending's `attemptId`. Pending resume adopts only a complete profile with that ID. A different profile gets typed `connect_profile_other_attempt`, names the profile and pending files and a new `--profile` path, makes no POST, and leaves all files unchanged. Setup refuses a directory with `connect-pending.json` before network access and again under the profile writer lock. | `Fold 13 pending profile must belong to this connect attempt` covers another agent's setup-shaped profile and a different attempt with the same agent; `Fold 12 pending resume keeps a host-bound working profile from this attempt` checks adoption, no POST, identical bytes, and matching IDs; `Fold 13 setup refuses a directory with a connect pending record before network` checks the typed refusal and unchanged pending. | Removing the attempt comparison: exit 1, targeted test 0/1 (foreign profile accepted). Removing setup's early guard: exit 1, targeted test 0/1 (unexpected network path). Both mutations were restored. |
