...
The original sections below record historical lane states. Fold 18 at the end is the current client rule; earlier credential deletion, mode, fresh-register escape, revocation-advice, and completed-profile rows are superseded where marked. Item M lands after lane 2b replaces the generic storage lock with one module, then receives cross-family review on top of lane 2b.
...
| HH6 — superseded in part by Fold 17 AE1 and Fold 18 AH2 | Damaged completion records still produce one warning and are ignored for selection when there is no pending record. Fold 7's unreadable-record warning and clear-removal claims are **superseded**: reserved-path preflight now refuses unreadable completion records, including on clear. Wrong-mode records get the exact chmod step. Clear can remove a readable completion record without pending, and pending removal must actually succeed. | The renamed `Fold 7 damaged completion warns; unreadable completion refuses before register and clear removes both` test now checks the typed refusal; Fold 18 also checks clear refusal before removal. | Historical Fold 7 mutations established the then-current behavior; Fold 18 measures clear classification separately. |
...
| KK3 — unsafe branch superseded by Fold 17 AE1 and Fold 18 AH5; post-POST wording extended by Fold 9 LL7 | A pending resume classifies the completion record under the connect lock. Wrong mode gives `chmod 600`; symlink or foreign owner now gives `connect_complete_unreadable` in preflight. The old `connect_complete_unsafe` branch and its test hook are removed. A completion write error gives `connect_complete_write_failed` and keeps the working profile. **Supersedes HH6's broad wrong-mode claim; the earlier claim was established only for the no-pending scan.** | `Fold 8 pending resume classifies completion and keeps a working profile on write failure` now covers mode, symlink, and injected EIO without another POST. | Historical Fold 8 mutation established the then-current behavior; Fold 18 removes the unreachable branch. |
...
|---|---|---|---|
| AE1 — extended by Fold 18 AH2/AH3/AH5 | `classifyConnectReservedPath` returns absent, ok, wrong mode, or unreadable. It checks the owned 0700 directory, owned regular 0600 file, and readable body. The preflight enumerates the fixed names and temporary patterns from `CONNECT_PROFILE_FILES` before locks, writes, or POSTs, on explicit paths and default discovery, with a second check under the connect lock. **Fold 18 also classifies clear's reserved paths before removal**, rechecks a path that vanishes during read, and gives custom profile basenames `connect_profile_*` codes. Wrong mode gives the exact `chmod` and same-command step; unsafe type, owner, or body names the file and inspection step. The lock writer sets 0600 explicitly so a restrictive umask cannot create a lock the preflight rejects. | `Fold 17 every reserved path refuses unsafe state before POST on explicit and default connect` covers 12 names × five states × two routes, using simulated owner and read faults; each cell checks zero POSTs and the exact repair or refusal sentence. `Fold 17 fresh connect checks marker before pending write or register` covers 0644 and symlink markers without pending, zero POSTs, and no pending write. Fold 18 tests cover clear, disappearing files, and custom names. | Removing the preflight calls: exit 1, 0/2 targeted tests at Fold 17; Fold 18 measures its extensions separately. |
...
## Fold 18 — Codex and Opus round 18 FAIL rulings

The reviews are `scratchpad/itemM/codex-r18.md` and `scratchpad/itemM/opus-r18.md`. Codex finding 1, the generic lock's empty-file crash window in `storage.ts`, is **deferred to lane 2b**. Lane 2b replaces that lock code with one module. Item M must land after lane 2b and be re-reviewed on top of it. This fold does not change the generic lock code. No server, migration, production host, or real workspace was touched.

| Ruling | Change | Behavioral test | Measured revert |
|---|---|---|---|
| AH1 | The reserved-path table sets `process.env.HOME` to its fixture before building default paths and restores it in `finally`. The lane control runs the entire MCP file under a new empty `/tmp/lane-home.*`, spies on `homedir()` calls from MCP path construction, lists the HOME entries afterward, and requires no entries and only fixture-derived homes. | `Fold 18 whole MCP lane leaves an empty isolated HOME and builds default paths only from fixtures` runs all 101 MCP tests as a child. | Removing the table's fixture HOME assignment, as at `6cac3114`: control exit 1, 0/1; isolated HOME contained `[".cswarm"]`. Restored. |
| AH2 | Clear preflights reserved names before and inside the connect lock. Immediately before each pending/completion unlink it classifies again; wrong mode gives the chmod step, unreadable gives a typed refusal, and absent is skipped. Clear never removes a file it could not read safely. | `Fold 18 clear classifies pending and completion before removal` covers wrong mode, oversized body, symlink, successful clear, and a mode change between removal of pending and completion. | Replacing the immediate classifier with a raw `lstat`: baseline 1/1, revert 0/1; the race removed a 0644 completion instead of refusing. Restored. |
| AH3 | A null read or `ENOENT` read error causes one more `lstat`; only a now-missing file is absent. | `Fold 18 a vanished reserved file is absent after the read race` checks both vanished and still-present outcomes. | Returning unreadable on the second `ENOENT`: baseline 1/1, revert 0/1. Restored. |
| AH4 | Both temp writers and the exclusive fallback call `fchmod(0600)` immediately after `open(0600)`, before invoking the body writer. | `Fold 18 temp writers set 0600 before writing under umask 0277` inspects each handle inside the write callback, including the fallback. | Moving the replacement writer's chmod after the body write: baseline 1/1, revert 0/1; the callback observed 0400. Restored. |
| AH5 | Removed the unreachable `connect_complete_unsafe` branch and `inspectCompletion` hook. The classifier recognizes the actual `--profile` basename and uses `connect_profile_*` codes. | `Fold 18 custom profile basename gets profile refusal codes` checks 0644 and symlink `agent.json` before POST. The completion test continues to check the reachable unreadable path. | Restoring the hard-coded `profile.json` code selection: baseline 1/1, revert 0/1; custom `agent.json` got `connect_reserved_mode`. Restored. |

The Fold 12 completion-failure injection now fails during the temp body write, before publication, because temp chmod correctly precedes that callback. Two historical clear tests now expect typed refusal for unsafe records and repair them before clearing. The timeout table cites the moved register timer at `mcp-connect.ts:787` and hidden-input deadline at `:317`.
...
| `npm run build` | Exit 0. |
| `npm run check:tests` | Exit 0. |
| `env HOME="$T" node --test-reporter=dot --import tsx --test tests/p1-cli/mcp-connect.test.ts tests/p1-cli/mcp-connect-home-control.test.ts` | Exit 0; 102/102 direct tests, with the control running the whole 101-test MCP file once more under a separate empty HOME. The outer HOME listing was empty. Both temporary HOME roots were removed. |

### Fold 18 not established

- The lead owns the full pure and CLI suites, edge check, release bundle, site build, and server gates at this fold's tip. No migration was applied.
- The generic storage lock's empty-file crash window remains for lane 2b and requires re-review after integration. No production host, real home, external filesystem, or real workspace was tested.
- `pgrep` could not inspect this sandbox's process list (`sysmond service not found`). The focused test commands and the control child exited; no task child was intentionally left running.
