...
| LL2 | Exclusive publication retains `link()` first. EPERM, ENOTSUP, EOPNOTSUPP, ENOSYS, or EXDEV falls back to `open(final, wx)`, write, chmod, fsync, close, then temp removal. A failed final write removes only its own inode. The competing-writer `EEXIST` result remains. **Extends KK6 and replaces the Fold 7 rename method.** | `Fold 9 unsupported hard links use exclusive final creation and recover a working profile` checks an interrupted attempt, same-attempt retry, one working profile, and each named failure family. `Fold 8 exclusive first writes publish exactly one credential` remains green. | Removing the fallback: baseline 1/1, mutation 0/1. |
| LL3 | Timeout mapping points to the final abort timer and `stty` source lines, 675 and 254. | `MCP register abort-timer citation points to the actual timer line`; `MCP hidden-input timeout citation points to the stty deadline`. | Restoring either stale citation independently: baseline 1/1, mutation 0/1 for each. |
| LL4 | Owned wrong-mode and unowned-ancestor diagnosis walks every absolute profile path to the filesystem root. **Extends KK4's home-only path scope.** | `Fold 9 permission diagnosis covers outside-home ancestors and unclassified denial` injects both ancestor cases outside HOME. | Restoring the home-only mode walk: baseline 1/1, mutation 0/1. |
...
| LL6 | An unclassified EACCES/EPERM becomes typed `connect_state_unavailable` with an inspection step. The CLI fallback maps any remaining raw connect permission error to that code. **Extends KK4's fallback.** | The Fold 9 permission test injects an otherwise unclassified denial and checks both CLI permission codes. | Restoring raw permission rethrow: baseline 1/1, mutation 0/1. |
| LL7 | After the profile save, a failed completion-record write says the profile is saved and usable, the record was not written, and the same command finishes the connect next run. **Extends KK3's resume-path wording to the first post-POST failure.** | `Fold 9 completion write failure reports a saved usable profile and resumes without POST` checks the message, profile, and no second POST. | Restoring `register_outcome_unknown`: baseline 1/1, mutation 0/1. |
