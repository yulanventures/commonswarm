...
| Ruling | Change | Focused test | Measured single-change revert |
|---|---|---|---|
| SS1 — production; extends QQ1 to pending records | `completedProfileAt` returns the validated profile read with its own `host_session_id`. The pending-record branch uses that result. After a crash before pending deletion, a setup-bound working profile is kept, pending is deleted, and no second POST or damaged-profile `mv` step occurs. | `Fold 12 pending resume keeps a setup-bound working profile` binds a profile, restores its same-code pending record, then checks zero resume POSTs, pending removal, and byte-identical profile. | Restoring the old branch that discards `completedProfileAt` and calls `readAgentProfile(path)` without the host ID: exit 1, targeted test 0/1. |
...
| SS3 — extends QQ2's clear wording | `--clear-pending` names both the empty credential claim and an unvalidated profile in the same directory and says both were kept. | `Fold 12 clear names a kept unvalidated profile beside an empty credential claim` checks both files and the clear output. | Restoring the earlier empty-claim-only sentence and rebuilding the CLI: exit 1, targeted test 0/1. |
