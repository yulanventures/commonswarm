...
| Ruling | Change | Focused test | Measured single-change revert |
|---|---|---|---|
| WW1 | A non-connect `saveAgentProfile` holds the connect lock around the setup lock. Connect's pending write also holds both, in that order. | `Fold 14 setup and connect serialize the pending check with the profile write` injects a pending write under the connect lock between setup's in-lock pending check and profile write. It observes that the write waits until setup finishes. | Removing the outer connect lock from setup: exit 1, 0/1; pending appeared during setup's write. Restored. |
| WW2 | `profile.json` keeps the released six-key connect shape. Connect writes the attempt ID to 0600 `connect-profile-attempt.json`, named in the reserved-file set, before publishing the profile. Pending adoption compares this marker; completion rebuild recreates it. Other profile readers do not open it. | `Fold 14 connect writes a v0.1.77-shaped profile and keeps its attempt marker private` applies the released exact-key reader and checks marker mode/ID. The Fold 12 bound adoption and Fold 13 foreign-profile refusals pass with the marker. | Putting `connect_attempt_id` back into the profile: exit 1, 0/1 in the released-reader test. Disabling the marker comparison: exit 1, 0/1 in the foreign-profile test; the foreign profile was adopted. Both restored. |
...
| WW5 | The `renderMcpConnect` comment now names the host session ID that bound install commands print. | `Fold 14 printed-line comment names the bound session field` pins the comment; the Fold 12 bound-install test checks the actual printed arguments. | Restoring the stale comment: exit 1, 0/1 in the comment check. Restored. |
| WW6 | The HEAD timeout-table row cites `agent-setup.ts:17,45-68`; the historical v0.1.71 row is unchanged. The displaced register timer citation is line 730. | `setup whole-operation timeout citation names the constant and deadline block` resolves all three cited lines; the existing register-timer test resolves its cited line. | Restoring the stale setup range: exit 1, 0/1. Restored. |
