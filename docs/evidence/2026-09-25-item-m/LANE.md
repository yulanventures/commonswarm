...
|---|---|---|---|
| ZZ1 — coverage extended by Fold 16 AB1/AB2 | A pending attempt's owned 0644 marker **beside a working profile** gets `connect_marker_mode` with its exact `chmod 600` path and same-command retry. Other unreadable markers get `connect_marker_unreadable` naming the marker. The marker cannot be silently treated as missing. **This Fold 15 read-path test did not cover a pending attempt without a profile or a completion rebuild; AB1 covers both writes. Its directory test reached the earlier directory check, not the marker catch; AB2 tests that catch's classifier.** | `Fold 15 wrong-mode attempt marker gives an exact repair and same-command resume` checks no POST, preserved profile, and successful retry after chmod. `Fold 15 unsafe attempt marker gets a typed file-specific refusal before POST` uses a symlink and checks the named marker and no POST. | Restoring the Fold 14 `.catch(() => null)` marker read: exit 1, 0/2. Both tests received the false `connect_profile_other_attempt` code. Restored. |
...
| Ruling | Change | Focused behavioral test | Measured single-change revert |
|---|---|---|---|
| AB1 | One owned-regular-file mode classifier runs before the pending request and before the completion rebuild writes the marker. A 0644 marker gives `connect_marker_mode`, its exact `chmod 600` path, and same-command retry. The pending path makes no POST. The read catch uses the same classifier. | `Fold 16 pending without profile classifies marker mode before register` checks zero retry POSTs, unchanged pending, and success after chmod. `Fold 16 completion rebuild classifies marker mode before rewriting it` checks zero POSTs, unchanged marker bytes, and local repair after chmod. | Removing both prewrite calls: exit 1, 0/2. The pending test got `register_outcome_unknown`; the rebuild test got a raw writer error without `connect_marker_mode`. Restored. |
| AB2 | The shared marker mode classifier tests the exact 0700 directory mode before the file mode. Pending, rebuild, and the read catch all call it. This makes the former unreachable directory branch independently callable and tested; a 0755 directory gets `connect_directory_mode` with `chmod 700`. | `Fold 16 marker read failure names a wrong-mode directory before an unsafe marker` uses a real 0755 directory and symlink marker, then checks the marker refusal after chmod. The test calls the same classifier as the three connect paths. | Removing the directory branch from that classifier: exit 1, 0/1; it returned `connect_marker_unreadable` instead of `connect_directory_mode`. Restored. |
| AB3 | Setup's concurrency test uses `refuseExisting=false`, as production setup does, and injects at the regular credential write. The outer connect lock still spans the pending check and write. | `Fold 14 setup and connect serialize the pending check with the profile write` checks pending cannot appear during the normal setup write. | Removing the outer connect lock: exit 1, 0/1; pending appeared during setup's write. Restored. |
