...
|---|---|---|---|
| ZZ1 | A pending attempt's owned 0644 marker gets `connect_marker_mode` with its exact `chmod 600` path and same-command retry. A wrong-mode directory gets `connect_directory_mode` with its exact `chmod 700` path. Other unreadable markers get `connect_marker_unreadable` naming the marker. The marker cannot be silently treated as missing. | `Fold 15 wrong-mode attempt marker gives an exact repair and same-command resume` checks no POST, preserved profile, and successful retry after chmod. `Fold 15 unsafe attempt marker gets a typed file-specific refusal before POST` uses a symlink and checks the named marker and no POST. The existing explicit-directory-mode test covers 0755. | Restoring the Fold 14 `.catch(() => null)` marker read: exit 1, 0/2. Both tests received the false `connect_profile_other_attempt` code. Restored. |
| ZZ2 | The HEAD storage citations resolve to `storage.ts:27` (`LOCK_TIMEOUT_MS`) and `storage.ts:385` (`timeoutMs`); the historical v0.1.71 rows are unchanged. The marker-read edit also moves the register timer citation to `mcp-connect.ts:742`. | `Fold 15 storage lock citations resolve to the constant and default` reads both cited source lines. The existing register-timer citation test checks line 742. | Restoring line 25 for the constant: exit 1, 0/1. Separately restoring line 383 for the default: exit 1, 0/1. Both restored. |
| ZZ3 | The comment-text assertion is removed. The setup/connect concurrency test injects through the credential file writer, so `saveAgentProfile` no longer has the test-only `afterPendingCheck` parameter. The bound-install behavior test remains. | `Fold 14 setup and connect serialize the pending check with the profile write` now observes the real file-write boundary. `Fold 12 pending resume keeps a host-bound working profile from this attempt` checks both printed install forms. | Removing the outer connect lock: exit 1, 0/1; pending appeared during setup's write. Restored. The removed comment assertion had no behavioral claim to mutate. |
