Fold 3 on item G lane 1 in the CommonSwarm repo, branch lane/item-g-lane1 at 321c5406 (your fold 2 + two lead
test-fixture commits: fad542e0 and 321c5406 — read them; they fix server-test setups you could not run). Spec:
docs/design/2026-09-24-ITEM-G-LANE-1-BRIEF.md with its corrections. The round-3 Opus review (FAIL) is pasted at the end.
Implement EVERY ruling with a test that fails when the fix is reverted; record them in
docs/evidence/2026-09-25-item-g-lane1/LANE.md under "Fold 3". Read AGENTS.md first.
...
The server tests run only on the lead's local stack: write them carefully (an acked delivery row needs delivered_at and
surfaced_at, as the edge writes; swarm.signals is append-only — use the disable-trigger pattern in
tests/p1-server/command.test.ts; mind fold 2's later-observed rule when you backdate rows).

RULINGS
I1 (Opus R3-1, PRODUCTION). Agents must get the stale receipt. One source, two layers: an ELIGIBILITY view (or SQL
   function) with no member check and NO grants to any client role, used by the SECURITY DEFINER receipt function; the
   roster read (`swarm_read.agent_wake_path`) adds `is_member(workspace, auth.uid())` on top of the same eligibility
   source. The receipt function keeps its own authorization (the caller may read receipts for its own signals) exactly
   as before. Server test: read the receipt of a stale row as the SENDING AGENT (agent credential through the read edge
   or the same function call path the edge uses) and as the owner; both see the stale state; a non-member sees nothing.
I2 (Opus R3-2, lead ruling). The view's release-cutoff filter is redundant with the known-seat rule (the seat's first
   observed ack is after the cutoff) plus the later-observed rule, so no test can make it matter. Keep it as a guard,
   state in the SQL comment and LANE.md that it is redundant by construction and why, and do not claim a test covers it.
I3 (Opus R3-3). The functional box proof must use the same eligibility source (no copied rules), so a later ack that
   hides the seed fails with the right message; test that failure mode.
I4 (Opus R3-4, lead ruling). No new index in this lane. Record the measured cost (Opus: 36 ms at 50k observed rows in one
   workspace) and the query plan in LANE.md as a follow-up; the lead asks HezLead for the production row count.
