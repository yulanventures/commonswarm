...
## Tom's mint block (before the window; any time 2026-09-25 18:00-21:30Z)

Tom signs in first with `cswarm login` in a terminal on the mini (the mint uses the human session and its device).
Then he pastes this block. It prints nothing but a final `OK`; every output goes into 0600 files in a 0700 directory;
no secret is in argv or history (the ids are not secrets).
...
(
  ...
  cswarm principal create --workspace-id "$WS" --name g-seed-sender-0925 >"$D/sender-principal.json"
  cswarm principal create --workspace-id "$WS" --name g-seed-recipient-0925 >"$D/recipient-principal.json"
  cswarm token mint --workspace-id "$WS" --principal-id "$(pid "$D/sender-principal.json")" \
    ...
    --ttl-ms 21600000 --renewal-horizon-days 1 >"$D/sender.json"
  cswarm token mint --workspace-id "$WS" --principal-id "$(pid "$D/recipient-principal.json")" \
    --run-id "$(uuidgen | lower)" --task-id "$(uuidgen | lower)" --epoch 1 \
    --ttl-ms 21600000 --renewal-horizon-days 1 >"$D/recipient.json"
  cp "$HOME/.config/cswarm/anon-key.txt" "$D/anon-key.txt"
  chmod 0600 "$D"/*.json "$D/anon-key.txt"
  echo OK
)
```
...
- Step 3: the read-edge probe with the recipient's credential, the exact body the CLI sends:
  ...
  (from a 0600 file, deleted after the call). Must return 200 and contain note 2. Failure exits 5 (`STOP step 3`).
...
| Step 1 STOP (the edge refused the ACK) or step 3 STOP | Roll back the EDGE only (to `4ef0f300`). The migration stays: rolling back the edge alone is safe (below). Then debug. |
| Step 4 (functional proof) fails, steps 1-3 passed | Keep the edge and the migration: both are safe, steps 1-3 already proved the new write and read paths, and the proof is read-only. The lead debugs with the proof's error text and the seed ids. HezLead may still roll back the edge; the SQL rollback runs only on HezLead's decision. |
...
**SQL (reserve; only on HezLead's decision).** `deploy/release-proofs/item-g/20260925000001-rollback.sql`, one
