...
|---|---|---|
| MM1 | `stop --wait` starts one 30-second deadline before the initial stop query. It accepts effective failed/absent state, and waits for both socket closure and PID exit after a live stop. The timeout includes state and PID. | The printed `stop --wait && start` line succeeded for slow stop, dead listener, and death during stop: 3/3. Restoring the old failed-state rejection made the dead branch fail 0/1. |
| MM2 | Host-id stale reclaim uses a unique rename behind an exclusive reclaim gate. Owner IDs distinguish same-process contenders with identical timestamps. | Two stale-lock contenders returned the one persisted host ID. Replacing the rename with unlink failed 0/1. |
| MM3 | The `ps` start-time probe fixes `LC_ALL=C`, and host-id lock timeout names owner PID. | A spawned child PID reached the injected ps runner, asserting executable, PID and locale; the live-owner timeout asserted PID. Removing `LC_ALL=C` failed 0/1. Actual `/bin/ps` execution is denied by this sandbox. |
...
| MM5 | Renewal-store reads use a read-only secure path. Invalid records fall back to the profile token with a reasoned snapshot and exit 0. | Malformed record, absent directory, unchanged bytes and mode passed. Restoring a thrown read error failed 0/1. |
...
| MM7 | Human and JSON `listen status` host labels use the lease-error sanitizer, including C1 and bidi removal. | Loopback status controls passed. Restoring the raw JSON label failed 0/1. |
| MM8 | HEAD timeout mapping now includes the listener stop poll and local `ps` timeout. The dispatcher fixture records the intended help line above. | Timeout table passed 19/19; removing the storage row failed its exact-inventory check 0/1. Removing the CLI poll row also failed that check 0/1. The dispatcher fixture had failed in the lead log and passed 2/2 after the regeneration. |
