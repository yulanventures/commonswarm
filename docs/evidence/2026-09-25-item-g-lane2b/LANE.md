...
| UU6 | The real `/bin/ps` control now compares the parsed PID start with the timestamp taken before spawning the child, within two seconds. | An independent positive/negative comparator control passed; replacing the spawn timestamp with the parsed timestamp failed 0/1. The real `/bin/ps` control skipped because this sandbox denied it with EPERM, so its integration still requires the lead's unrestricted gate. |
...
- The lead has not yet run Fold 13's full service-free, CLI, server, edge, site, or release gates. Fold 12's lead gates were red as stated above. The real `/bin/ps` spawn-time control is skipped in this sandbox; the independent comparator mutation was measured, but real process inspection remains unmeasured here.
