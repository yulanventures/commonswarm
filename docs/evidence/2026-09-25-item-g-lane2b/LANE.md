...
## Merge with main after item M (2026-09-26)

The intentional merge commit `eaa76148` combined main `c29d4779` (including item M) with lane 2b `511d7e22` and retained conflict markers for this integration. Before the repair, `git grep -nE '^(<<<<<<<|=======|>>>>>>>)( |$)'` found markers in six tracked files: two hunks in `src/cloud/storage.ts`, one in `src/cli.ts`, one in `src/cloud/agent-profile.ts`, two in `scripts/timeout-table/mapping.json`, one in `tests/p1-cli/timeout-table.test.ts`, and 337 hunks in the recorded dispatch fixture. After the repair, the same tracked-file scan returned no matches.

### Conflict resolutions

- `src/cloud/storage.ts`: kept lane 2b's complete-owner publication, reclaim gate, creation-time reuse decision, aged-unreadable reclaim, and rename-then-verify removal as the only generic lock path. Item M's old pre-publication `open("wx")` path was not retained because it would create a second lock inode before the shared publisher ran. The first M control found a real composed regression under `umask 0277`: lane 2b's temp inode became `0400`, so M's reserved-path check refused its own `mcp-connect.lock`. The explicit `chmod(0600)` now runs inside `publishCompleteOwnerFile`, before bytes are written. This keeps M's 0600 lock behavior while applying it to every shared lock publication; M's separate exclusive credential fallback, 0600 file writes, 0700 directory checks, and crash recovery remain unchanged. That control exercised hard-link publication only. The exclusive symlink fallback was unreachable through `connectMcp` until the follow-up fix below added a narrow publisher seam and taught preflight to recognize the lock module's own symlink.
- `src/cli.ts`: retained item M's `privatePath` import for `mcp connect --clear-pending` output and lane 2b's `verifiedLiveSessionContexts` import for watcher/session fencing.
- `src/cloud/agent-profile.ts`: retained item M's `CONNECT_PROFILE_FILES` and `writeSecureJsonFileExclusive` paths and lane 2b's `listSessionContextFiles` path. The merged save flow therefore keeps M's pending-connect and exclusive-claim recovery while every setup/connect lock goes through lane 2b's shared `withFileLock` implementation.
- `scripts/timeout-table/mapping.json`: kept item M's MCP rows already present around the conflict and lane 2b's distinct `pidStartMs` plus generic-lock rows. After the symlink-lock fix, the storage citations are `storage.ts:35,631-632`, `storage.ts:528`, and `storage.ts:631`; they name the final constant, PID-start probe, and lock default respectively.
- `tests/p1-cli/timeout-table.test.ts`: kept both sides' citation controls. Item M's old storage assertion was adjusted because the composed inventory assigns `timeoutMs` to lane 2b's earlier PID-start probe and `timeoutMs#2` to the generic lock default; the assertion still checks the same generic lock expression under its resolved inventory id. No behavioral assertion was weakened.
- `tests/p1-cli/fixtures/command-dispatch-baseline.json` and `command-dispatch-baseline-counts.json`: took main's parent fixtures as the starting point, then regenerated both through the required recorder after the final build. The count remains 1,330 rows: 41 zero and 1,289 nonzero exits.

### Regenerated rows different from both parents

Exactly 337 rows differ from both parent rows. In every case the only changed field is `stdout` or `stderr`, and that field contains the global usage text. The composed usage has lane 2b's `inbox --notify ... --session-context ... --take-over` and `listen stop ... --state-dir ... --wait` lines, while also retaining main's `mcp connect ... --clear-pending`, `file put ... --request-id`, and `brain put ... --request-id` lines. Thus it cannot byte-match either one-sided parent. The following compact Cartesian notation lists every differing row; braces mean every named alternative and `P` is the exact 12-name prototype set shown below.

`P = {constructor, __defineGetter__, __defineSetter__, hasOwnProperty, __lookupGetter__, __lookupSetter__, isPrototypeOf, propertyIsEnumerable, toString, valueOf, __proto__, toLocaleString}`

- `refusal.unknown-verb` (1).
- `refusal.prototype-verb.P.{plain,json}` (24).
- `refusal.group.hook.{missing,unknown}.{plain,json}` (4).
- `refusal.group.{listen,session,channel,file,brain}.{missing,unknown}.{plain,json,profile-valid,profile-valid-json}` (40).
- `refusal.group.grant.unknown.{plain,json}` (2).
- `refusal.group.link.{missing,unknown}.{plain,json}` (4).
- `refusal.prototype-sub-action.hook.P.{plain,json}` (24).
- `refusal.prototype-sub-action.{listen,session,channel,file,brain}.P.{plain,json,profile-valid}` (180).
- `refusal.prototype-sub-action.{grant,link}.P.{plain,json}` (48).
- `meta.{help-flag,help-verb,no-positional-json,no-positional-profile,bare}` (5).
- `policy.host-session.{brain.refusal.drop,channel.refusal.drop,file.refusal.drop,listen.refusal.keep,session.refusal.keep}` (5).

The enumeration reconciles as `1 + 24 + 4 + 40 + 2 + 4 + 24 + 180 + 48 + 5 + 5 = 337`. No row differs from both parents in exit code, argv, or handler trace.

### Verification

The final focused checks all used the required wrapper and its temporary HOME:

- `host-id-rotation.test.ts`: 36 passed, 0 failed, 2 real-`ps` controls skipped by the sandbox; `/tmp/alloy-g2b-host-id-rotation-rerun.test.ts.log`.
- `mcp-connect.test.ts`: 101/101 passed after the shared publisher mode fix. That run did not force `link()` to fail and therefore did not exercise the exclusive symlink fallback.
- `mcp-connect-home-control.test.ts`: 1/1 passed; `/tmp/alloy-g2b-mcp-connect-home-control.test.ts.log`.
- `timeout-table.test.ts`: 25/25 passed after final repinning; `/tmp/alloy-g2b-timeout-table-final.test.ts.log`.
- `citation-drift.test.ts`: 5/5 passed; `/tmp/alloy-g2b-citation-drift-final.test.ts.log`.
- `command-dispatch-baseline.test.ts`: the final recorder/control passed 2/2; `/tmp/g2b-baseline-final.log`.
- `npm run build`, `npx tsc -p tsconfig.tests.json --noEmit`, and `git diff --check` passed on the final source; the final tracked-file conflict-marker scan was empty.

### Connect preflight and the symlink lock (after the merge)

Defect: `publishCompleteOwnerFile` falls back to a symlink when hard links are unavailable, but connect's inner reserved-path preflight rejected every symlink, including the `mcp-connect.lock` held by that same invocation. The earlier merge tests never passed `publishLink`, so this fallback was not reached.

Fix: `connectMcp` now exposes only the existing lock publisher seam to tests. The reserved-path classifier accepts `mcp-connect.lock` and `setup.lock` symlinks only when the link target is a regular same-directory file owned by the current user, mode `0600`, with the publisher's exact `<lock name>.<pid>.<16 lowercase hex>.tmp` name. `removePublishedOwnerFile` and connect preflight share one exported target-name validator. Other symlinks remain unreadable; lock publication, reclaim, and removal behavior did not change.

Measured before: on the `556b84a2` classifier, after adding only the narrow `publishLink` plumbing needed to reach the fallback, the connect integration forced `link()` to return `EPERM`. Its inner preflight failed at `mcp-connect.lock` with `connect_reserved_unreadable`; the named test failed 0/1.

Measured after: the same integration completed one registration, returned one working profile, observed the held lock as a symlink to a regular file, and left no lock behind. The complete connect file passed 104/104. Separate controls accepted both owned lock names and refused an external-directory target, a mode-`0644` target, a target with the wrong name, and a symlink at a non-lock reserved path.

Tests: the required acceptance script exited 0. It built the CLI and typechecked tests, then ran every test through `scripts/run-gates.sh` with a temporary home: `mcp-connect.test.ts` 104/104, `mcp-connect-home-control.test.ts` 1/1, `host-id-rotation.test.ts` 36 passed and 2 sandbox-dependent controls skipped, `timeout-table.test.ts` 25/25, and `citation-drift.test.ts` 5/5. The existing `umask 0277` connect control is part of the 104/104 result. No deployment or live verification was performed.
...
The gate claims one lease as host `g2b-renew-gate`, then runs 50 ordered renew/check pairs. It calls the built `dist/cloud/wake-lease.js` and `dist/cloud/agent-check.js` modules without timeout overrides. Before every lease call it reopens the built profile credential session and asks for its current bearer; if a check rotates and retires the minted token, the next renew adopts the stored successor. `G2B_RENEW_GATE_ROUNDS=3` is the local-test shortening seam; leave it unset in the box window. Any run shorter than exactly 50 pairs is explicitly a rehearsal: its last line and report headers say `GATE NOT A RELEASE GATE n=<k> <PASS|FAIL-reason>`, and it exits 10 regardless of its measurements.

The gate writes mode-`0600` `renew-gate.json` and `renew-gate.md` in the seed directory. The JSON retains every call, including the cold first renew and check; both reports show n, first-call time, p50, p95, and max, and both put the exact terminal gate line in their header. The timeout table records the shipped 15,000 ms renew timeout at `src/cloud/wake-lease.ts:64` and the non-gating 3,900 ms check budget at `src/cloud/agent-check-budget.ts:22`. It also writes mode-`0600` `renew-gate-state.json` so the exact held lease can be released. Standard output contains only IDs, statuses, timings, and stable error codes; it prints `G2B_PRINCIPAL_ID=<id>` for the functional SQL proof and never prints the credential. An edge error is retained only when it matches `[a-z][a-z0-9_]{0,63}`; every other edge-supplied error string becomes `unrecognized_error` in output and reports.

Exit codes are:

- `0`: `GATE PASS` after exactly 50 pairs; every renew succeeded and renew p95 is below 15,000 ms. With `--release`, the recorded lease was released.
...
- `7`: `GATE FAIL <reason>` after exactly 50 pairs because a renew failed or renew p95 was not below 15,000 ms. Reports and the held-lease state remain available.
...
- `10`: `GATE NOT A RELEASE GATE n=<k> <PASS|FAIL-reason>` because fewer than 50 pairs were requested. The reports and held-lease state remain available, but this result is never release evidence.
