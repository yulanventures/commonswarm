...
| AF4 | The CLI records claim, renewal-loss, release and takeover observations. It prints “nothing is watching” only after `released: true`; a failed or `released: false` response preserves the last-known-holder warning. Release 409 selects watcher or H0 poll. | Loopback release-true, release-false, 503 and both 409 surfaces passed. Unconditionally treating a response as released or ignoring the 409 notice each failed a focused control. |
