...
The preceding Fold 9 mutation results were measured before the incident and preserved in commit `897d58fb`; recovery reran the fixed-source controls, not the destructive source mutations. The five-second citation currently points to `timeoutMs: 5_000` at `live-session-context.ts:43`, and the machine-id citation points to the `ioreg` timeout at `arrival-watch.ts:259` (Fold 15 line). The earlier `:255` reference is superseded. The existing timeout-table check cannot detect a citation that names a wrong but existing line.
...
| XX4 (gate-age start superseded by Fold 15 AC3) | Gate and host-id lock check PID start even after EPERM. If `ps` is unavailable, an uninspectable owner is reclaimable after `HOST_ID_LOCK_MAX_HOLD_MS` (60 seconds). The Fold 14 gate measured from prepublication `createdAt`; a legacy gate without that field uses directory mtime. | Fresh uninspectable owner timed out; aged lock and gate were reclaimed. Disabling each age fallback separately failed 0/1. The stop wait retains its one deadline. |
...
|---|---|---|
| AC1 | Seat and legacy watcher acquisition is serialized by a per-path complete-owner gate. A stale lock is renamed to a unique path, and only a moved record matching the observed dead one is removed before publication. A changed record is restored. The young incomplete-file wait shares the five-second acquisition deadline. Dead-PID cleanup removes a killed publisher's watcher `.tmp`. | Two takeovers of each dead lock left one watcher; a moved-record replacement was preserved; a future-dated partial lock stopped at the deadline; a SIGKILLed publisher's private temp was removed by the next start. Removing the gate changed the paired takeover control from pass to exit 1. |
| AC2 | An incomplete gate is abandoned only when a second stat finds the same device and inode as the first. | Injected release-and-replace between stat and owner read retained the new gate and timed out. Removing the inode condition changed pass to exit 1. |
| AC3 | New gate records mark rename publication; the uninspectable-owner age uses the published directory's ctime, not the pre-rename record timestamp. Legacy records retain their previous age rule. | A fresh gate with a prepublication timestamp more than 60 seconds old was retained. Reverting the publication-time choice changed pass to exit 1. |
| AC4 | The general credential lock publishes its complete owner through `publishCompleteOwnerFile`, like the host-id and watcher locks. | A paused writer left the credential lock path absent. Reverting to exclusive final-path open and write changed pass to exit 1. |
| AC5 | Signal-stop copy uses the known holder: this watcher, another watcher, H0 poll, or an unclaimed lease. The CLI passes claim and typed lease-loss state to the sentence builder. | Copy tests checked this-watcher and other-surface text; loopback held-lease and preclaim signal tests passed. Reverting the other-watcher sentence changed the copy control from pass to exit 1. |
| AC6 | The wake-lease release timeout row cites `cli.ts:5011-5017` and `wake-lease.ts:60,63`; the machine-id line reference is `arrival-watch.ts:259`. | The citation test reads the mapped lines and checks the release command, two-second timeout, parameter and abort timer. Restoring the old citation changed pass to exit 1. |

The six mutation probes each ran one focused test with temporary HOME and returned exit 1; the fixed controls returned exit 0. Final scoped checks: `npm run build` exit 0, `npm run check:tests` exit 0, `git diff --check` exit 0; focused arrival-watch 6/6, host-id rotation 4/4, and timeout-table 2/2 passed. Focused loopback signal tests also passed: arrival-notify 2/2, wake-lease CLI 4/4, resume 2/2. The lead owns the full gates. Fold 14's XX1 takeover, XX3 incomplete-gate decision, and XX4 gate-age start claims above are superseded by AC1–AC3.

## Not established

- Fold 15's full service-free, CLI, server, edge, site and release gates await the lead. The lead ran Fold 13's gates at 80a6915f; its CLI gate was red on XX2's mapping defect. The new supervisor-versus-real-`ps` control is not measured outside this sandbox yet.
