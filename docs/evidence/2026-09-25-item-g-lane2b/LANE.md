...
| MM5 (scope superseded by Fold 12 PP6) | All successor-store reads used the read-only path, including renewal readers. Resume handled invalid records with a reasoned snapshot and exit 0. | Malformed record, absent directory, unchanged bytes and mode passed for resume. The control did not exercise renewal's directory repair. |
...
|---|---|---|
| PP1 | `stop --wait` reads the stored status without calling the mutating fallback. A timed-out control query stays unknown. Completion requires refused socket connection and the recorded PID's absence or start-time mismatch; a recorded `stopped` state returns immediately. The one deadline reports state and PID. | A real child and a 300 ms control reply kept `stopping` unchanged until both socket and child exited. A live stuck child reached the bounded timeout with its PID and unchanged status. A simulated reused PID returned while that child lived. Restoring the mutating fallback failed 0/1 (`failed` instead of `stopping`); disabling start-time comparison failed 0/1 on the reused PID. |
| PP2 | The reclaim gate is a directory with a synced owner record containing PID and process start time. A live gate obeys the caller's deadline and names its path; a dead owner's gate is moved away and removed. | A spawned child owned the gate; its live gate timed out with `.reclaim` in the message. After the child died, the same stale lock was reclaimed and a loopback watcher started through that path. Disabling dead-gate recovery failed the lock control 0/1 on the abandoned gate and the watcher control 0/1 on its test deadline. |
| PP3 | The wait controls use live children. Stale takeover has a hook after the stale decision, letting a second owner publish before the move. The ps test invokes real `/bin/ps` on a spawned child and compares parsed start times. | Replacing rename with unlink failed the publication-race control 0/1 because the second owner's lock vanished. `/bin/ps` returned sandbox `EPERM` here, so that one control skipped; it remains for the lead's unrestricted gate. |
...
| PP6 | Resume alone selects the read-only successor-store reader. Renewal reads repair an owned 0755 directory before the lock and surface malformed records instead of silently adopting the presented token. | Resume retained 0755 and fell back with a reason; a renewal session repaired 0755, adopted the stored successor, and rejected a malformed record. Reverting resume to the repairing reader failed 0/1 on the mode, and reverting renewal to the read-only reader failed 0/1 on the stored read. |
...
- The lead's green `npm test` 1018/1018 and `test:p1-cli` 1008/1008 are at b4919452, before Fold 12. Fold 12's full service-free, CLI, server, edge, site, and release gates remain for the lead.
- The real `/bin/ps` start-time comparison was attempted and skipped after sandbox `EPERM`; the lead must run it where process inspection is permitted. The lane's existing read-only resume process-table test also fails here on sandbox `spawn EPERM`.
- Fold 12's `pgrep -fl` check exited 3 because this sandbox could not reach `sysmond`; host-wide process absence is not established here. Every new child test kills and awaits its child in cleanup.
