...
| F4-3 | `rate_limited` names the hourly limit for this agent or its workspace (correction, lead after Opus round 4: the edge returns the same 429 `rate_limited` for the per-credential limit, 120 per hour, and the per-workspace limit, 1000 per hour, `supabase/functions/command/index.ts` `SIGNAL_CREDENTIAL_LIMIT` / `SIGNAL_WORKSPACE_LIMIT`; the fold's sentence "The signal rate limit for this agent was reached" was false for the workspace limit) and tells the caller to wait and retry with the same `request_id`. The command client does not type or parse the edge body's `resets_at`, so no `retry_after` field was added. | The fake edge pins the full 429 payload. Restoring the prior sentence and step: selected test exit 1, 0/1 pass. |
