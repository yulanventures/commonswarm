...
The upgrade checks the four fixed migration hashes and the target identity. With the two join tables and the two poll tables absent, it applies the four migrations in order and checks the join catalog in one transaction. With the join tables present and the poll tables absent, it applies `20260922000001` and `20260922000002` in that order, in one transaction. With all four tables present, it checks the join catalog and skips SQL migration replay. A partial or malformed catalog fails; do not retry by deleting tables or editing checksums. The post-upgrade verifier preserves every original table count and cron record. It adds the two join tables at zero when the source baseline lacked both, and the two poll tables at zero when the source baseline lacked both. It retains a pair's original counts when a recovery snapshot already contained that pair. An extra or missing table fails. There is no fixed total-table count. The retired wording said the verifier added only the two join tables.
