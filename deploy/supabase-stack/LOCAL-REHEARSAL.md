...
| Dump | Exit 0. Custom dump size 7,077,101 bytes. Selected 7 schemas, 84 table counts, and 273 `swarm-files` object rows. `realtime.messages` data was excluded. |
...
| Storage copy | Exit 0. Copied and SHA-256 checked 273 objects, 6,873 bytes total. A second run also passed, proving idempotent upsert behavior. |
...
- Production service versions were not read. The committed pins are the local CLI versions and must be replaced only if the lead's public health checks differ.
