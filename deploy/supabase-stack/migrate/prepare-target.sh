...
assert_target_identity >>"$LOG_FILE" 2>&1
log "target identity accepted before activity publish grants"
target_psql --file "$MIGRATE_DIR/activity-publish-grants.sql" >>"$LOG_FILE" 2>&1
log "commonswarm_edge has realtime USAGE and INSERT (id, payload, event, topic, private, extension) under commonswarm_edge_activity_insert"
