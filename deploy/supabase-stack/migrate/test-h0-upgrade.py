...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='commonswarm-h0-test-') as temp:
        ...
        try:
            ...
            def reset():
                check(sql('DROP SCHEMA IF EXISTS swarm_read CASCADE; DROP SCHEMA IF EXISTS swarm CASCADE; DROP SCHEMA IF EXISTS auth CASCADE;', 'h0_fixture'), 'reset')
            ...
            check(sql("CREATE SCHEMA cron; CREATE TABLE cron.job(jobid int,jobname text,schedule text,command text,database text,username text,active boolean)", 'h0_fixture'), 'synthetic cron catalog')
