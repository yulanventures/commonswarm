...
# Prints only ids, statuses and exit codes; never a token. Exit codes: 0 pass; 2 bad input files; 4 STOP (the edge
# did not accept the unclaimed observed ACK); 5 STOP (the read-edge probe failed); 6 an unexpected CLI failure.
set -euo pipefail
...
# 3. Read-edge probe with the recipient's credential: the body the CLI sends; 200 and it contains note 2.
python3 - "$DIR" "$ANON" <<'PY'
import json, os, sys
d, anon = sys.argv[1:3]
tok = json.load(open(os.path.join(d, "recipient.json")))["agent_token"]
...
CODE="$(curl -sS -o "$DIR/read-resp.json" -w '%{http_code}' -X POST -H @"$DIR/read-headers.txt" --data @"$DIR/read-body.json" "$URL/functions/v1/read")"
