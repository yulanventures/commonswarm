-- RESERVE ONLY. Rolls back migration 20260925000002_renewal_last_use_monotonic by restoring the two functions it
-- replaced, exactly as supabase/migrations/20260904000001_standing_grant_resume.sql defined them (bodies, owner,
-- grants and comment copied verbatim). Runs only on HezLead's decision, with the section-5 write helper
-- (`release_psql --file /proof/20260925000002-rollback.sql`). Rolling back brings back the production race that
-- raises SWARM_RENEWAL_LAST_USE_REWOUND (the read edge's intermittent HTTP 500).
\set ON_ERROR_STOP 1
BEGIN;
...
DELETE FROM supabase_migrations.schema_migrations WHERE version = '20260925000002';
COMMIT;
